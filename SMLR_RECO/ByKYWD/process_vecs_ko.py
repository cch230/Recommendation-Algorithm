...
def is_hangul(text) -> bool:
    return bool(re.match(r'^[0-9\u3130-\u318F\uAC00-\uD7A3]+$', text)) and len(text) > 1
...
if __name__ == '__main__':
    connection = sqlite3.connect('../DataSet/valid_guesses_ko.db')
    cursor = connection.cursor()
    cursor.execute("""CREATE TABLE IF NOT EXISTS guesses (word text PRIMARY KEY, vec blob)""")
    print("created table")
    normal_words = load_dic('../DataSet/ko_filtered.txt')
    print("# words in dictionary:", len(normal_words))
    valid_nearest = []
    valid_nearest_mat = None
    eliminated = 0
    checked_words = set()
    total_lines = count_lines('../DataSet/cc.ko.300.vec') - 1
    with open('../DataSet/cc.ko.300.vec', 'r', encoding='utf-8', errors='ignore') as w2v_file:
        _ = w2v_file.readline()
    ...
    with open('../DataSet/valid_nearest_ko.dat', 'wb') as f:
