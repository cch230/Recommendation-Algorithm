...
data = df_shuffled.drop(['category'], axis=1).values
...
# 그라데이션 색상 설정
gradient = np.linspace(0, 1, len(vectors_tsne))
colors = [instagram_gradient_colors[int(x * (len(instagram_gradient_colors) - 1))] for x in gradient]

# 시각화
plt.figure(figsize=(12, 10))
plt.xticks(np.arange(-100, 101, 5))
plt.yticks(np.arange(-100, 101, 5))
plt.scatter(vectors_tsne[:, 0], vectors_tsne[:, 1], c=colors)
plt.title('t-SNE 2D Visualization of Vectors')
plt.savefig('TSNE_2D_with_instagram_gradient.png', bbox_inches='tight')
plt.show()
