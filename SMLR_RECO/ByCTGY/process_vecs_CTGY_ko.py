...
def is_hangul(text) -> bool:
    return bool(re.match(r'^[0-9\u3130-\u318F\uAC00-\uD7A3]+$', text))
...
if __name__ == '__main__':
    connection = sqlite3.connect('../DataSet/CTGY_guesses_ko.db')
    cursor = connection.cursor()
    cursor.execute("""CREATE TABLE IF NOT EXISTS guesses (word text PRIMARY KEY, vec blob)""")
    print("created table")
    normal_words = load_dic('../DataSet/output_oneElement.txt')
    ...
    with open('../DataSet/CTGY_nearest_ko.dat', 'wb') as f:
