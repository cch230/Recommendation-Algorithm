...
# CSV 파일 읽어오기
df = pd.read_csv('../DataSet/category_groupBy_user_20230825.csv', dtype=np.float32)
df_shuffled = df.sample(frac=1, random_state=24)  # frac=1은 전체 데이터를 의미하며, random_state는 재현성을 위한 시드값입니다
data, test_data = train_test_split(df_shuffled, test_size=0.5, random_state=245)
...
def evaluate_n_trees(n_trees):
    ...
    _, distances = annoy_index.get_nns_by_vector(random_row, 20, include_distances=True)
    # print(distances)
    total = sum(distances)
    count = len(distances)
    average = total / count
...
pbounds = {'n_trees': (1, 1000)}
optimizer = BayesianOptimization(f=evaluate_n_trees, pbounds=pbounds, random_state=542)
...
distance_values = [round(abs(p['target']), 5) for p in sorted_data if p['target'] != -2.0]
n_trees_values = [round(p['params']['n_trees']) for p in sorted_data if p['target'] != -2.0]
